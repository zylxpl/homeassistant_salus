...
| SQ610 / SQ610RF | Quantum thermostat | `climate`, humidity sensor, floor temperature sensor when reported, diagnostic binary sensors, optional `lock` | Target temperature, HVAC mode, preset mode, schedule/manual/off, cool mode when reported, child lock when reported | Schedule is exposed as preset `Follow Schedule`, not HVAC `Auto`. |
| FC600 | Fan-coil controller | `climate`, humidity sensor, diagnostic binary sensors, optional `lock` | Target temperature, HVAC mode, preset mode, fan mode, schedule/manual/eco/off, child lock when reported | Supports fan modes and heat/cool operation when reported by the gateway. |
